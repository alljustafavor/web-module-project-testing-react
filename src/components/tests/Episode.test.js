...
test("renders without error", () => {
    ...
    const img = screen.getByAltText(exampleEpisodeData.image);
    expect(img).toBeInTheDocument();
});

test("renders the summary test passed as prop", () => {
    render(<Episode episode={exampleEpisodeData} />);
    const summery = screen.getByText(exampleEpisodeData.summary);
    expect(summery).toBeInTheDocument();
});

test("renders default image when image is not defined", () => {
    render(<Episode episode={exampleEpisodeDataWOIMG} />)
    const img = screen.getByAltText('https://i.ibb.co/2FsfXqM/stranger-things.png');
    expect(img).toBeInTheDocument();
});
